...
export default class reviewsController{
    ...
    static async addReview(req,res){
        try{
            ...
            if(!product_id || !rating || !comment) return res.status(400).json({succeeded: false, message: "Không được bỏ trống các trường"});

            const result = await reviewsModel.addReview({rating: rating, comment: comment}, product_id, user_id);
